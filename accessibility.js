(function () {
  ...
  // Text-size is controlled as a multiple of 10 (10% steps)
  const DEFAULT_SCALE = 100; // percent
  const MIN_SCALE = 70; // 70%
  const MAX_SCALE = 130; // 130%
  const STEP = 10; // 10%
  ...
  function setScale(scale) {
    ...
    // Apply scaling to typography only.
    // We inject CSS that multiplies base font-size by --font-scale.
    document.documentElement.style.setProperty('--font-scale', String(s));
    // Scaling happens via the injected CSS rule for `body`.
    // (We store the value + update the label; no inline font-size mutations.)
